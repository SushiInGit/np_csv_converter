...
var _fileType = 0;
var sessionDUMP =  sessionStorage.getItem('excelData');
const parsedSessionData = JSON.parse(sessionDUMP);
logger.log(parsedSessionData);
...
function handleFileUpload(files) {    
    ...
    if (file && file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
        ...
        fileUploadModal.classList.remove("shown");
        document.getElementById("welcomeDIVcontainer").style.display = "flex";
        document.getElementById("head-container").style.display = "flex";
        document.querySelector("#headTable").style.display = "flex";
        document.getElementById('searchInput').value = 'flex';
